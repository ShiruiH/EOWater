...
# load polygon layer
epsg = 32755
fp_polygons = os.path.join(os.getcwd(),'shared_with_NGIS','OFS_layer_NGIS','NorthernValleys_OFS_forNGIS.geojson')
...
fp_ngis = os.path.join(fp_data,'Shirui_S2_1tile')
list_files = os.listdir(fp_ngis)
df_ngis = pd.DataFrame()
for file in list_files:    
    fn = os.path.join(fp_ngis,file)
    df = pd.read_csv(fn, parse_dates=['date'])
    df_ngis = pd.concat([df,df_ngis], axis=0)
df_ngis['satname'] = ['S2' for _ in range(len(df_ngis))]

# load NGIS L5
fn_file = os.path.join(fp_data,'L5_TOA_OFS_Vali_1986_2013_28Y_newExp.csv') 
...
# load NGIS L7
fn_file = os.path.join(fp_data,'L7_TOA_OFS_Vali_1999_2023_25Y_newExp.csv') 
fn = os.path.join(fn_file)
...
# load NGIS L8
fn_file = os.path.join(fp_data,'L8_TOA_OFS_Vali_2013_2023_11Y_newExp.csv') 
fn = os.path.join(fn_file)
...
fp_ngis_ts = os.path.join(os.getcwd(),'NGIS_data','ts_NGIS_newExp_TOA')
...
for site_id in site_ids:
    ...
    df_site.to_csv(os.path.join(fp_ngis_ts,'%s_area_timeseries.csv'%site_id),index=False)