// #####################################################
// Script to process L8 (2013-2024)
// #####################################################
...
// dates
var dates = ['2013-03-18', '2025-01-01'];
...
var allValleysOFS = baseOFS.map(function(eachTile){
  ...
  var collection1 = ee.ImageCollection("LANDSAT/LC08/C02/T1_TOA") // L8_TOA
                    .filter(criteria)
                    .map(maskl8toa)
                    .map(function(image){
  ...
  var singleRegionFC = binaryNDWI.map(function(singleImage){
    ...
    var finalOFS_FC = ee.FeatureCollection(ks.map(function(key){
      ...
      return ee.Feature(ee.Geometry.Point(0.0, 0.0), {'date': singleImageDate, 'system_time_utc': singleImageSysDate, 'UNIQUEID': ky, 'count': vl, 'area': area, 'tile': ee.String(ee.Number(path).int()).cat('_').cat(ee.Number(row).int())});
...
// main loop for exporting the tasks
for (var x = 0; x < allValleysOFS.size().getInfo(); x ++){

  var outputFC = allValleysOFS.toList(allValleysOFS.size()).get(x)
  var aoiName = 'L8_OFS_2013_2024_11Y_' + 'Tile_' + ee.List(tileList).get(x).getInfo();
